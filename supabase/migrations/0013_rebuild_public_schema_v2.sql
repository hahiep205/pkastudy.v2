-- Rebuild plan for a clean Supabase public schema v2.
-- Intended for a fresh database or for a full reset of the public schema after backup.
-- Goals:
-- - one ownership model per entity
-- - consistent uuid-based user references
-- - RLS first, not as a patch
-- - index-driven scaling
-- - safe to rerun in an empty/recreated public schema
...
-- Indexes for scale.
create unique index if not exists uq_profiles_legacy_user_id
...
create index if not exists idx_user_word_progress_flashcard_id
  on public.user_word_progress (flashcard_id);

create index if not exists idx_srs_reviews_user_id
  on public.srs_reviews (user_id);

create index if not exists idx_srs_reviews_next_review_date
  on public.srs_reviews (next_review_date);

create index if not exists idx_toeic_question_groups_test_part
  on public.toeic_question_groups (test_id, part);

create index if not exists idx_toeic_questions_test_part
  on public.toeic_questions (test_id, part);

create index if not exists idx_toeic_test_records_user_id
  on public.toeic_test_records (user_id);

create index if not exists idx_support_tickets_user_id
  on public.support_tickets (user_id);

create index if not exists idx_vocab_activity_logs_user_id_created_at
  on public.vocab_activity_logs (user_id, created_at desc);
