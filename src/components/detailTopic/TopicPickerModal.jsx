...
export default function TopicPickerModal({ isOpen, onClose, word, customCourses, onAdd, onCreateTopic }) {
    ...
    const filteredTopics = useMemo(() => {
        ...
        return allTopics
            ...
            .sort((a, b) => {
                const aCount = Array.isArray(a.words) ? a.words.length : 0;
                const bCount = Array.isArray(b.words) ? b.words.length : 0;
                return bCount - aCount;
            });
    }, [allTopics, searchQuery]);

    const firstAvailableTopic = useMemo(
        () => filteredTopics.find((topic) => !topic.words.some((item) => normalizeWord(item.word) === normalizeWord(word?.word))),
    ...
    const getDuplicateTopic = (topicId) => {
        ...
        const isDuplicate = target.words.some((item) => normalizeWord(item.word) === currentWordKey);
            ...
            <div className="cv-topic-picker-box" id="cv-topic-picker-box" onClick={(event) => event.stopPropagation()}>
                ...
                <div className="cv-modal-body cv-tp-body">
                    <div className="cv-tp-word-pill">
                        ...
                        <span className="cv-tp-word-tag">
                            {allTopics.length} danh sách
                        </span>
                    </div>
                    ...
                    {message ? (
                        <div className={`cv-tp-message is-${message.type}`}>
                            {message.text}
                        </div>
                    ) : null}
                    ...
                    {allTopics.length === 0 ? (
                        ...
                        <div className="cv-tp-topic-list">
                            {filteredTopics.map((topic) => {
                                ...
                                const isDuplicate = topic.words.some((item) => normalizeWord(item.word) === currentWordKey);
                ...
                <div className="cv-modal-footer cv-modal-footer-split cv-tp-footer">
                    ...
                    >
                        {selectedTopicDuplicate
                            ? 'Danh sách này đã có từ'
                            : !selectedTopic
                                ? 'Chọn danh sách'
                                : 'Thêm vào'}
                    </button>
